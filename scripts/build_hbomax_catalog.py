"""
Build HBO Max catalogs: data/hbomax_movies.json and data/hbomax_series.json
Fetches latest 100 HBO Max releases (.hmax.1080) from nCore, matches to TMDB only (no Trakt).

Usage: python scripts/build_hbomax_catalog.py
Output: data/hbomax_movies.json, data/hbomax_series.json
"""
import re
import time
import os
...
out_file_movies = data_dir / 'hbomax_movies.json'
out_file_series = data_dir / 'hbomax_series.json'
...
# Fetch 100 latest HBO Max items
TARGET_COUNT = 100
...
def load_existing_movies():
    """Load existing hbomax_movies.json if present."""
...
def load_existing_series():
    """Load existing hbomax_series.json if present."""
...
def fetch_hbomax_movies(client, max_count=None):
    """
    Fetch latest HBO Max movies from nCore (.hmax.1080 pattern, HD_HUN category).
    ...
    all_torrents = []
    page = 1
    print(f"🎬 Fetching HBO Max movies (.hmax.1080) from nCore (max {target})...")
...
def fetch_hbomax_series(client, max_count=None):
    """
    Fetch latest HBO Max series from nCore (.hmax.1080 pattern, HDSER_HUN category).
    If max_count is set (incremental mode), stop after that many torrents.
    """
    ...
    all_torrents = []
    page = 1
    print(f"📺 Fetching HBO Max series (.hmax.1080) from nCore (max {target})...")
    while len(all_torrents) < target:
        torrents = None
        for attempt in range(1, NCORE_PAGE_RETRIES + 1):
            try:
...
def main():
    ...
    movie_torrents = fetch_hbomax_movies(client, max_count=max_count)
    print(f"\n✓ {len(movie_torrents)} HBO Max film torrent letöltve nCore-ról")
    ...
    for idx, t in enumerate(movie_torrents, 1):
        ...
        description = metadata['description'] or 'HBO Max film – nCore legfrissebb feltöltés.'
        year = metadata['year']
    ...
    if incremental_movies:
        print(f"\n✓ {len(merged_movies)} HBO Max film katalógus frissítve: {out_file_movies} (+{len(new_movie_metas)} új)")
    else:
        print(f"\n✓ {len(merged_movies)} HBO Max film katalógus írva: {out_file_movies}")

    # ========== SERIES ==========
    print("\n" + "=" * 60)
    print("📺 HBO MAX SOROZATOK")
    print("=" * 60)
    ...
    series_torrents = fetch_hbomax_series(client, max_count=max_count)
    print(f"\n✓ {len(series_torrents)} HBO Max sorozat torrent letöltve nCore-ról")
    ...
    for idx, t in enumerate(series_torrents, 1):
        ...
        display_poster = f'https://image.tmdb.org/t/p/w500{poster_path}' if poster_path else f"https://images.metahub.space/poster/small/{imdb_id}/img"
        genres_list = metadata['genres']
        description = metadata['description'] or 'HBO Max sorozat – nCore legfrissebb feltöltés.'
        year = metadata['year']
        rating = metadata['rating']
    ...
    if incremental_series:
        print(f"\n✓ {len(merged_series)} HBO Max sorozat katalógus frissítve: {out_file_series} (+{len(new_series_metas)} új)")
    else:
        print(f"\n✓ {len(merged_series)} HBO Max sorozat katalógus írva: {out_file_series}")

    print("\n" + "=" * 60)
    print("✅ HBO Max katalógusok sikeresen elkészültek!")
    print("=" * 60)
