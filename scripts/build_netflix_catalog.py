"""
Build Netflix catalogs: data/netflix_movies.json and data/netflix_series.json
Fetches latest 100 Netflix releases (.nf.1080) from nCore, matches to TMDB only (no Trakt).

Usage: python scripts/build_netflix_catalog.py
Output: data/netflix_movies.json, data/netflix_series.json
"""
import re
import time
import os
...
out_file_movies = data_dir / 'netflix_movies.json'
out_file_series = data_dir / 'netflix_series.json'
...
# Fetch 100 latest Netflix items
TARGET_COUNT = 100
...
def load_existing_movies():
    """Load existing netflix_movies.json if present."""
...
def load_existing_series():
    """Load existing netflix_series.json if present."""
...
def parse_series_title(title):
    ...
    if year_match:
        ...
        cut_pattern = re.search(
            r'\.(S\d+|E\d+|\d{3,4}p|WEB-?DL|HDTV|BluRay|BRRip|DVDRip|PROPER|REPACK|AAC|DD\+?|DV|HDR|H\.26[45])',
            title,
            re.IGNORECASE
        )
        if cut_pattern:
...
def fetch_netflix_movies(client, max_count=None):
    """
    Fetch latest Netflix movies from nCore (.nf.1080 pattern, HD_HUN category).
    ...
    all_torrents = []
    page = 1
    print(f"🎬 Fetching Netflix movies (.nf.1080) from nCore (max {target})...")
...
def fetch_netflix_series(client, max_count=None):
    """
    Fetch latest Netflix series from nCore (.nf.1080 pattern, HDSER_HUN category).
    If max_count is set (incremental mode), stop after that many torrents.
    """
    ...
    all_torrents = []
    page = 1
    print(f"📺 Fetching Netflix series (.nf.1080) from nCore (max {target})...")
    while len(all_torrents) < target:
        torrents = None
        for attempt in range(1, NCORE_PAGE_RETRIES + 1):
            try:
...
def main():
    ...
    movie_torrents = fetch_netflix_movies(client, max_count=max_count)
    print(f"\n✓ {len(movie_torrents)} Netflix film torrent letöltve nCore-ról")
    ...
    for idx, t in enumerate(movie_torrents, 1):
        ...
        description = metadata['description'] or 'Netflix film – nCore legfrissebb feltöltés.'
        year = metadata['year']
    ...
    if incremental_movies:
        print(f"\n✓ {len(merged_movies)} Netflix film katalógus frissítve: {out_file_movies} (+{len(new_movie_metas)} új)")
    else:
        print(f"\n✓ {len(merged_movies)} Netflix film katalógus írva: {out_file_movies}")

    # ========== SERIES ==========
    print("\n" + "=" * 60)
    print("📺 NETFLIX SOROZATOK")
    print("=" * 60)
    ...
    series_torrents = fetch_netflix_series(client, max_count=max_count)
    print(f"\n✓ {len(series_torrents)} Netflix sorozat torrent letöltve nCore-ról")
    ...
    for idx, t in enumerate(series_torrents, 1):
        ...
        display_poster = f'https://image.tmdb.org/t/p/w500{poster_path}' if poster_path else f"https://images.metahub.space/poster/small/{imdb_id}/img"
        genres_list = metadata['genres']
        description = metadata['description'] or 'Netflix sorozat – nCore legfrissebb feltöltés.'
        year = metadata['year']
        rating = metadata['rating']
    ...
    if incremental_series:
        print(f"\n✓ {len(merged_series)} Netflix sorozat katalógus frissítve: {out_file_series} (+{len(new_series_metas)} új)")
    else:
        print(f"\n✓ {len(merged_series)} Netflix sorozat katalógus írva: {out_file_series}")

    print("\n" + "=" * 60)
    print("✅ Netflix katalógusok sikeresen elkészültek!")
    print("=" * 60)
